...
OniStatus Rs2Device::startPipeline()
{
	...
	for (;;)
	{
		...
		// really bad workaround to get full path
		char path[MAX_PATH];
		HMODULE hm = NULL;

		if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
			GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
			(LPCSTR)&doNothing, &hm) == 0)
		{
		...
		std::string fullPath = std::string(path).substr(0, pos);
		// add json file to path
		fullPath.append("\\realsense.json");

		// added by Ryan Pennings
		// TODO: Make it not shit...
		rsLogDebug("rs_load_json");
		//std::ifstream jcontent("C:\\realsense.json", std::ifstream::binary);
		std::ifstream jcontent(fullPath, std::ifstream::binary);
		if (!jcontent.good())
		{
			throw std::runtime_error("Failed to read configuration file:\n\"");
		}
		std::string str((std::istreambuf_iterator<char>(jcontent)), std::istreambuf_iterator<char>());
		rs2_load_json(m_device, str.data(), (unsigned int)str.size(), &e);
		if (!e.success())
		{
			rsTraceError("rs2_load_json failed: %s", e.get_message());
			break;
		}
